...
requires:
  - phase: 04-pretool-hook
    provides: "sekha.hook.main() PreToolUse entrypoint + sekha.cli hook run dispatch (from 04-01)"
provides:
  - "sekha.hook.bench() 100-run subprocess benchmark with p50/p95/p99 + budget-based exit codes"
  - "tests/test_hook_bench.py — SEKHA_BENCH-gated CI perf gate mirroring Phase 2 pattern"
...
affects: [05-mcp-server, 06-sekha-init, ci-matrix, v0.1.0-release, future-perf-regressions]
...
tech-stack:
  ...
  patterns:
    ...
    - "Env-var budget overrides with platform-aware defaults (mirrors Phase 2 SEKHA_BENCH_P95_MS)"
    - "SEKHA_BENCH gate on slow perf tests — fast CI stays fast, nightly/release CI enforces"
...
key-files:
  ...
  modified:
    - src/sekha/hook.py  # added bench() function
...
patterns-established:
  - "bench() functions live inside their module (sekha.hook.bench) with lazy imports, dispatched via CLI and also exercised by a SEKHA_BENCH-gated unittest"
  - "Bench fixtures live under tests/fixtures/<component>_rules/ and are copied into a per-run temp SEKHA_HOME for isolation"
...
**`sekha hook bench` 100-subprocess p50/p95/p99 gate (HOOK-08) + `docs/hook-integration-test.md` manual Claude Code dogfooding runbook (HOOK-10)**
...
- **`sekha.hook.bench()`** — 100-run subprocess benchmark of `python -m sekha.cli hook run`, reports `p50=<n>ms p95=<n>ms p99=<n>ms runs=100 budget_p50=Xms budget_p95=Yms` to stderr, exits 1 on budget breach. Lazy-imported so `sekha hook run` cold-start stays pristine.
- **`tests/test_hook_bench.py`** — SEKHA_BENCH-gated unit test asserting p50≤50ms AND p95≤150ms (win32 default p95 relaxed to 300ms; both overridable via `SEKHA_HOOK_P50_MS` / `SEKHA_HOOK_P95_MS`). Fast-discover suite skips correctly (216 tests, 2 skipped).
...
2. **Task 2: SEKHA_BENCH-gated test** — `0f3f28e` (test)
...
### `python -m sekha.cli hook bench` (Task 1 manual run)

```
p50=127.9ms  p95=143.8ms  p99=167.3ms  runs=100
budget_p50=50ms  budget_p95=150ms
sekha hook bench: BUDGET EXCEEDED   (p50 over on Windows — expected)
```

### `SEKHA_BENCH=1 python -m unittest tests.test_hook_bench -v` (Task 2)
...
**With Windows-appropriate overrides `SEKHA_HOOK_P50_MS=200 SEKHA_HOOK_P95_MS=300`:**
...
Windows Python cold-start is **~100–250 ms baseline before a single line of Sekha code runs** (documented in Phase 2 research). Our measured p50 of ~130 ms means Sekha itself adds only ~20–30 ms on top of the interpreter launch — the `-X importtime` budget from plan 04-01 is holding. Linux / macOS numbers are expected to sit comfortably inside the tight 50 / 150 budget when CI runs on those platforms; the Windows runner will need the overrides documented below.

## Recommended CI Budget Overrides

| Runner                     | `SEKHA_HOOK_P50_MS` | `SEKHA_HOOK_P95_MS` | Rationale                                                   |
| -------------------------- | ------------------- | ------------------- | ----------------------------------------------------------- |
...
- `src/sekha/hook.py` — added `bench()` function (~95 lines, all imports local)
...
- `tests/test_hook_bench.py` — SEKHA_BENCH-gated unit test (104 lines)
...
- **Warm-up run excluded from the sample.** First invocation hits the cold filesystem cache for `sekha.cli`, `sekha.hook`, and the rule fixtures — including it biases p50 upward by ~30–50 ms on Windows. One warmup = 1% overhead per bench run, and produces a stable distribution.
- **Fixture path via three `.parent` hops from `src/sekha/hook.py`.** Explicit "bench fixtures missing" error if the layout ever changes — fails loud rather than silently benchmarking an empty rules dir.
- **Invoke `-m sekha.cli` rather than the `sekha` console script** inside bench. On Windows the `.exe` shim adds 20–30 ms of variance (CONTEXT.md specifics section). `-m sekha.cli` is reproducible across dev boxes and CI runners.
- **Windows default p95 relaxed to 300 ms, p50 stays at 50 ms.** p95 relaxation is a platform reality (mirrors Phase 2 search-bench). Keeping the p50 default at 50 ms means the test fails on Windows by default — that's intentional: contributors on Windows must consciously set `SEKHA_HOOK_P50_MS=200` (or equivalent), which forces them to look at the measured number and catch any non-Windows regression they might have introduced.
...
Minor note on rule-fixture frontmatter: the plan spec includes a `name:` field in each fixture's frontmatter, but `sekha.rules._parse_rule_file` derives rule names from the filename (not frontmatter). The `name:` key is accepted and ignored, so the fixtures work exactly as specified. No code change needed.
...
- **Phase 6 (sekha init)** will automate the manual runbook steps — copy rule, edit `~/.claude/settings.json`. The runbook's exact steps are the spec for what `sekha init` must do.
...
- `src/sekha/hook.py` (modified) — FOUND
