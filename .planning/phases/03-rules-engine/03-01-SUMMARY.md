...
dependency-graph:
  requires:
    - sekha.storage.parse_frontmatter  # Plan 01-02
    - sekha.paths.sekha_home            # Plan 01-01
    - sekha.paths.category_dir          # Plan 01-01
    - sekha.logutil.get_logger          # Plan 01-01
  provides:
    - sekha.rules.Rule
    - sekha.rules.load_rules
    - sekha.rules.evaluate
    - sekha.rules.test_rule
    - sekha.rules.clear_cache
    - sekha._rulesutil._compile_rule_pattern  # re-exported for Phase 5 validation
  affects:
    - Phase 4 (PreToolUse hook — imports load_rules + evaluate)
    - Phase 5 (MCP server sekha_add_rule — reuses _compile_rule_pattern)
tech-stack:
  added: []                 # zero new deps
  patterns:
    ...
    - Lazy import (sekha.rules.Rule inside _parse_rule_file) to break circular
...
Pure-logic rules matcher — `sekha.rules.load_rules(dir, event, tool)` + `evaluate(rules, tool_input)` — that the Phase 4 PreToolUse hook can import without worrying about correctness, backed by a mtime-keyed compile cache, SEKHA_PAUSE env override, and 52 tests across 13 fixtures.
...
| 2    | feat     | `2161e58` | feat(03-01): implement sekha._rulesutil helpers + Rule dataclass stub       |
| 3    | test     | `12a7660` | test(03-01): add failing tests + 13 fixtures for sekha.rules API            |
| 4    | feat     | `b44827e` | feat(03-01): implement sekha.rules public API (RULES-01..08)                |
| 5    | chore    | `d483192` | chore(03-01): traceability comment + __all__ declaration                    |

## What Shipped

### `sekha._rulesutil` (private helpers — 125 lines)
...
### `sekha.rules` (public API — 180 lines)

- `Rule` — frozen dataclass with tuple-typed `triggers`/`matches` for hashability.
- `load_rules(dir, event, tool) -> list[Rule]` — cache-aware disk read, filters to `(event in triggers) AND (tool in matches OR "*" in matches)`, applies `SEKHA_PAUSE` suppression.
...
- `test_rule(name, tool, tool_input) -> dict` — dry-run; reads `~/.sekha/rules/<name>.md` directly, bypasses cache, raises `FileNotFoundError` on missing file.
- `clear_cache()` — drops `_CACHE` wholesale.
- `__all__` tuple documents the 5-name public surface for `from sekha.rules import *`.
...
| -------- | --------------------------------------------------------------------- | ---------------------------------------------- |
| RULES-01 | `_parse_rule_file` + `sekha.storage.parse_frontmatter` reuse          | `TestParseRuleFile`, `TestLoading`             |
...
| RULES-08 | `_paused_names()` reads `SEKHA_PAUSE` every call                      | `TestPause` (4 tests)                          |
...
- **Issue:** Plan's Task 3 test scaffolding used `contextlib.redirect_stderr(io.StringIO())` to capture the stderr warnings emitted by `sekha.logutil.get_logger(...).error(...)`. The two stderr-capture tests (`test_invalid_rules_logged_loudly_to_stderr`, `test_tie_breaks_by_first_in_list_and_logs_both_names`) failed because `sekha.logutil` configures the `StreamHandler` with `sys.stderr` at logger-setup time — the handler holds a direct reference, not a dynamic lookup, so the context manager's `sys.stderr` rebinding never reaches the handler.
- **Fix:** Switched both tests to `self.assertLogs("sekha.rules", level=...)` — the idiomatic unittest path for logger-output capture. Same bytes captured; different (and more reliable) capture path.
...
None. Every exported surface is fully wired and tested. `_compile_rule_pattern` is re-exported as a `# noqa: F401` private helper — intentional: Phase 5's MCP `sekha_add_rule` will consume it to validate user-supplied regex before writing rule files.
...
- No `print()` calls; all diagnostic output via `sekha.logutil.get_logger` → stderr.
...
### For Phase 5 (MCP `sekha_add_rule`)

Use `sekha._rulesutil._compile_rule_pattern(raw, anchored=...)` to validate a user-supplied regex before writing the rule file. It raises `re.error` on bad input — convert that to a friendly MCP error message.
...
$ python -c "from sekha.rules import Rule, load_rules, evaluate, test_rule, clear_cache; print('ok')"
ok
...
$ grep 'print(' src/sekha/rules.py src/sekha/_rulesutil.py
# (empty)

$ grep 'os\.path' src/sekha/rules.py src/sekha/_rulesutil.py
# (empty)
```
...
- [x] `src/sekha/rules.py` — created (189 lines, `__all__` exports 5 names)
- [x] `src/sekha/_rulesutil.py` — created (140 lines, 5 private helpers)
