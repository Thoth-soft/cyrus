...
Build the bedrock library every other component depends on — atomic markdown file storage with hand-rolled frontmatter, cross-platform filelock, and the `~/.sekha/` directory taxonomy. Zero user-visible value, but every downstream phase breaks if this has bugs.

Delivers: `sekha.storage`, `sekha.paths`, `sekha.logutil` — three pure-stdlib modules with a thoroughly-tested public API. No MCP, no hook, no CLI in this phase.
...
### Package Layout
- Code lives in `src/sekha/` (per existing src-layout scaffolding)
...
- **Taxonomy:** Exactly 5 fixed category folders under `~/.sekha/`: `sessions/`, `decisions/`, `preferences/`, `projects/`, `rules/`. Enforced by API — no arbitrary categories.
- **Filename format:** `YYYY-MM-DD_<id>_<slug>.md` where `<id>` is an 8-char hash and `<slug>` is lowercase-hyphenated title (max 40 chars).

### Paths Module (`sekha.paths`)
- `cortex_home()` returns `Path` pointing to `~/.sekha/` by default
- `SEKHA_HOME` env var overrides — useful for testing and portable installs
...
- `SEKHA_HOME` pointed at `tempfile.mkdtemp()` for isolation — each test gets a fresh sandbox
...
- Whether to add a `sekha.io` subpackage for file ops or keep flat (suggest flat — only 3 modules)
...
### Reusable Assets
- `src/sekha/__init__.py` — already exports `__version__`
- `src/sekha/py.typed` — PEP 561 marker, no changes needed
...
### Integration Points
- `~/.sekha/` directory rooted via `sekha.paths.cortex_home()` — all file I/O goes through this
- Downstream modules (`search`, `rules`, `hook`, `server`) will import from `sekha.storage` and `sekha.paths`
...
- Filename convention **must** be grep-friendly (per research ARCHITECTURE.md): `grep -rl '<tag>' ~/.sekha/` should work
...
- `sekha list-categories` CLI (Phase 6)
